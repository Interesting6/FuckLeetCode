...
class Solution:
    def sortList(self, head: ListNode) -> ListNode:
        # 后续，先分裂成一个个小的，最后merge这些小的成大的
        def merge(p1: ListNode, p2: ListNode):
            dummy_head = ListNode(-1)
            cur = dummy_head
            while (p1 and p2):
                if (p1.val < p2.val):
        ...
        def sort(head: ListNode, tail: ListNode):
            if not head:
                return head
            if head.next == tail:
                head.next = None # 截断
...
"""
Accepted
28/28 cases passed (496 ms)
Your runtime beats 25.18 % of python3 submissions
Your memory usage beats 70.59 % of python3 submissions (30 MB)
"""
